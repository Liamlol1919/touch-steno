...
| **Twiddler** | one-handed chord sequence | thumb + 1–3 fingers | key switches | CHI/HCI studies report learning and one-handed chording; values vary by study/user. | Medium: map discrete taps/postures | 5-key vocabulary is learnable but not automatically fast for an arbitrary surface. |
...
| **TypeAnywhere** | finger-tap sequence | fingers + thumb chords | finger/wearable sensors | 44.6 WPM belongs to cited TOAST, not TypeAnywhere itself; do not conflate | Medium–high | architecture reference for tap sequences + LM |
