...
The exact metrics remain historical facts. The valid decision is narrow: the current Contact Field implementation is rejected; Elastic remains a control/fallback; FCPT remains arithmetic-only with its selectable-thumb primitive rejected. The later repaired Test 0 comparator and failed TSF falsifier are recorded separately; `RCI_FALSIFIER_DECISION.md` now selects RCI as the single fresh design-only branch, while SOS and other directions remain held. No branch or hardware claim is promoted.
