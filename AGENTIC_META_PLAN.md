...
| **Temporal Set-Flow (TSF)** | Encode ordered non-rigid deformation of the whole anonymous field after ephemeral adjacent-set assignment and rigid translation/rotation/scale nuisance removal. No persistent identity or selected writer. | **BOUNDED PROTOTYPE `IMPLEMENTED`, RESEARCH-ONLY; FROZEN SYNTHETIC FALSIFIER REJECTED/HELD.** All 11 TSF tests pass, but `run_falsifier()` gives positive accuracy `0.0` over 12 test examples, accuracy interval `[0.0, 0.0]`, null false-commit rate `0.0`, null interval `[0.0, 0.0]`, threshold `0.0834054855946207`, best control `rigid_motion` at `0.3333333333333333`, observed margin `-0.3333333333333333`, and paired margin interval `[-0.6666666666666666, 0.0]`. `kill_gate_passed=false`; claim is `none`. | Do not tune or advance this prototype. Preserve the design rationale and exact failure in `TSF_FALSIFIER_RESULTS.md`; require a fresh post-failure branch decision before authorizing any replacement. |
| **Rest-Censored Innovation (RCI)** | Recognize a bounded excursion of the anonymous field from locked, per-session set rest. | **LATER COMPARATOR / HOLD; UNIMPLEMENTED.** TSF's failure does not automatically select or authorize RCI. | Consider only after a fresh post-failure branch decision. It requires matched slow-motion, palm, and rest-drift falsification. |
| **Spatiotemporal Occupancy Sketch (SOS)** | Encode local grid-space occupancy and motion over a bounded temporal window. | **LATER COMPARATOR / HOLD; UNIMPLEMENTED.** TSF's failure does not automatically select or authorize SOS. | Consider only after a fresh post-failure branch decision. It requires proving temporal locality beats static occupancy and occupancy-difference controls. |
...
frozen in `TSF_FALSIFIER_RESULTS.md`. The failed run creates no product branch and does not
authorize TSF retuning or automatic promotion of RCI/SOS.
