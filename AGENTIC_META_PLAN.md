...
| **Contact-field / permutation-invariant gesture representation** | Represent a gesture by the shape of its anonymous contact field, aiming to remove contact-ID and finger-order sensitivity while retaining coupled-field information. | **REJECTED AS CURRENT PRIMARY REPRESENTATION; REDESIGN/HOLD.** The frozen `SYNTH-BRANCH-1` run reports macro recall `0.585`, coverage `0.8`, wrong-commit rate `0.28`, conditional accepted-event error `0.35`, null upper95 `0.013347160654775625`, and nuisance recall `0.9625/0.7625/0.3375/0.0` (clean/mild/moderate/severe). | Redesign the representation and declare a fresh evidence boundary before any new comparator or claim. Do not retune thresholds to hide failed safety/nuisance gates. |
| **Continuous elastic word-as-event recognition** | Treat one continuous trajectory as an elastic word event, with normalization and confidence-gated rejection rather than letter-by-letter decoding. | **SYNTHETIC CONTROL/FALLBACK / HOLD.** The frozen comparator reports macro recall `1.0`, coverage `0.8888888888888888`, wrong-commit rate `0.0`, conditional accepted-event error `0.0`, and nuisance recall `1.0` in every stratum. It is not end-to-end and is not product-promoted. | Define and test anonymous-contact segmentation, path construction, recorder integration, and a versioned correction policy before any operational fallback claim. |
...
The earlier static weighted comparison is historical context, not the current branch decision. The frozen result is recorded in `SYNTH_BRANCH_1_RESULTS.md`; no branch is promoted to product. Synthetic comparator results are not hardware, human, WPM, correction, or text-accuracy evidence.
