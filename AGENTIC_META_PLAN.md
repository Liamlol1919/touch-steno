...
| **Scientific comparator repair / Test 0** | Validate decision isolation, independent split families, count matching, all-train aggregation, calibration-only selection, integrity controls, and session-clustered uncertainty. | **IMPLEMENTED, TESTED, AND FROZEN; SYNTHETIC EVIDENCE-BOUNDARY ONLY.** Canonical `FROZEN_MANIFEST_SHA256` is `922524b495753bd2f4394380b850f4d788c4b38b9af4ea057e57414fe601a25f`; raw file SHA-256 is `eccee106fab4b7aa19a48d0f160c4ea4a933e74b794b996da39a980747aa77c4`. Read-only `load_frozen_manifest()` validates it at runtime and refuses mutation with `ValueError`. All 9 comparator tests and the full 286-test suite pass (1 skipped, 0 failed). Exact metrics and limitations are frozen in `SCIENTIFIC_COMPARATOR_TEST0_RESULTS.md`. | Comparator freeze is complete. Test 0 is not a TSF, hardware, human, performance, or product result. |
| **Temporal Set-Flow (TSF)** | Encode ordered non-rigid deformation of the whole anonymous field after ephemeral adjacent-set assignment and rigid translation/rotation/scale nuisance removal. No persistent identity or selected writer. | **BOUNDED PROTOTYPE `IMPLEMENTED`, RESEARCH-ONLY; FROZEN SYNTHETIC FALSIFIER REJECTED/HELD.** All 11 TSF tests pass, but `run_falsifier()` gives positive accuracy `0.0` over 12 test examples, accuracy interval `[0.0, 0.0]`, null false-commit rate `0.0`, null interval `[0.0, 0.0]`, threshold `0.0834054855946207`, best control `rigid_motion` at `0.3333333333333333`, observed margin `-0.3333333333333333`, and paired margin interval `[-0.6666666666666666, 0.0]`. `kill_gate_passed=false`; claim is `none`. | Do not tune or advance this prototype. Preserve the design rationale and exact failure in `TSF_FALSIFIER_RESULTS.md`; the fresh replacement decision is now RCI only. |
| **Rest-Censored Innovation (RCI)** | Recognize a bounded excursion of the anonymous field from an explicitly captured, pooled empirical set-rest measure locked before classification, using a scalar permutation-invariant energy-distance innovation curve. No per-contact baseline, adjacent matching, selected writer, or TSF residual flow. | **SINGLE FRESH DESIGN-ONLY BRANCH; UNIMPLEMENTED.** `RCI_FALSIFIER_DECISION.md` freezes the exact primitive, immutable robust location/scale, strict return/settle lifecycle, independent generator families, matched count/nuisance/null bank, controls, metrics, hard vetoes, and `RCI-SYNTHETIC-FALSIFIER-1` gates. No manifest, implementation, test result, or performance result exists. | Implement only this frozen RCI design next, then freeze `RCI-SYNTHETIC-FALSIFIER-1` before test inspection. Preserve decisions-before-truth, calibration-only entry/acceptance selection, all-train exemplar use, independent split implementations, count-only chance, immutable rest, and session-clustered uncertainty. Any hard-gate failure yields `claim=none`; TSF is not revived or tuned. |
| **Spatiotemporal Occupancy Sketch (SOS)** | Encode local grid-space occupancy and motion over a bounded temporal window. | **HOLD / UNIMPLEMENTED.** It is not the selected branch and receives no parallel implementation, promotion, or performance credit. | Do not implement in the RCI branch. A later decision would still have to prove temporal locality beats static occupancy and occupancy-difference controls. |
...
`SYNTH-BRANCH-1` remains explicitly **non-promotional and structurally audited**. Its exact values remain historical facts, but favorable null/safety and perfect Elastic claims receive no promotion credit. Full audit findings, hard vetoes, TSF's primitive/state/observation model, falsifier, and exact kill criteria remain frozen in `POST_FAILURE_BRANCH_DECISION.md`. The repaired Test 0 comparator is now implemented, tested, and frozen: runtime `load_frozen_manifest()` validation enforces canonical digest `922524b495753bd2f4394380b850f4d788c4b38b9af4ea057e57414fe601a25f` and refuses mutation with `ValueError`; 9 comparator tests and the 286-test full suite pass (1 skipped, 0 failed). Decisions are isolated from truth and split metadata, the always-positive control fails the null gate, train/calibration/test use independent generator families, positive cardinality is matched, all training exemplars are aggregated, calibration alone selects the threshold, integrity controls fail as required, and confidence intervals use

The bounded TSF prototype advanced from `DESIGN` to research-only `IMPLEMENTED` after its
structural tests, then failed the first frozen synthetic falsifier. Its canonical manifest
digest is `a95f0d18832362b314417c557d615be4802cc869010617113fb75100090ffe7e`.
The exact failure, controls, confidence intervals, gates, and synthetic-only limitations are
frozen in `TSF_FALSIFIER_RESULTS.md`. TSF remains held/rejected and is not tuned or revived.

The fresh post-failure decision selects **RCI only** as a `DESIGN / UNIMPLEMENTED` research
branch and holds SOS. `RCI_FALSIFIER_DECISION.md` is the complete design and falsifier
authority. It is not an implementation, synthetic pass, promotion, or hardware claim; its
exact future gates require serialized decisions before truth join, independently implemented
train/calibration/test generators, all permitted train exemplars, calibration-only
entry/acceptance selection, matched counts and nuisances, count-only chance,
session-clustered uncertainty, immutable rest, and `claim=none` after any hard-gate failure.
